...
@section('content')
    ...
    <div class="max-w-[1200px] mx-auto mt-[54px] flex">
        ...
        <div class="w-full p-4">
            <div class="container mt-5">
                <h1 class="mb-4 text-center text-primary font-bold">List Banner Kecil</h1>
    ...
    <script>
        // Fungsi untuk mengambil data banner kecil dari API
        async function fetchBannerKecil() {
            try {
                const response = await fetch('/api/banner-kecil'); // Pastikan endpoint API Anda benar
        ...
        async function deleteBannerKecil(id) {
            ...
            try {
                const response = await fetch(`/api/banner-kecil/${id}`, {
                    method: 'DELETE',
                });
        ...
        function renderTable(banners) {
            const tableBody = document.getElementById('bannerKecilTable');
            ...
            if (banners.length === 0) {
                ...
                tableBody.innerHTML = `
                    <tr>
                        <td colspan="4" class="text-center">Tidak ada data banner kecil.</td>
                    </tr>
                `;
                return;
            }
            ...
            banners.forEach((banner, index) => {
                ...
                row.innerHTML = `
                    ...
                    <td>${banner.text || 'No Title'}</td>
                    <td>
                        <img src="${imagePath}" alt="${banner.tittle}" class="h-32 w-32 object-cover rounded-lg">
                    </td>
                    <td class="text-center">
                        <a href="/form-banner-kecil?id=${banner.id}" class="btn btn-warning btn-sm">Edit</a>
        ...
        // Panggil fungsi fetchBannerKecil saat halaman dimuat
